...
function epochToJsDate(epochTime){
  ...
  const setupUI = (user) => {
    if (user) {
      ...
      var chartRange = 0;
      // Get number of readings to plot saved on database (runs when the page first loads 
      //and whenever there's a change in the database)
      ...
      function appendToTable(){
        ...
        dbRef.orderByKey().limitToLast(100).endAt(lastReadingTimestamp).once('value', function(snapshot) {
          ...
          if (snapshot.exists()) {
            ...
            lastReadingTimestamp = dataList[0].timestamp; //oldest timestamp corresponds to the 
            //first on the list (oldest --> newest)
