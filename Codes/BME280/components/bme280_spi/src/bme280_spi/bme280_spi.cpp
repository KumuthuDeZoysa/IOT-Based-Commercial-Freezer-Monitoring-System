...
namespace CPPBME280
{
    ...
    int BME280SPI::getStatus()
    {
        return readByteData(STATUS);
    }

    int BME280SPI::getCalibrateData()
    {
    ...
    int BME280SPI::getSensorData(SensorRawData *resultRaw)
    {
    ...
    float BME280SPI::compensateTemp(const signed long adc_T)
    {
        int32_t var1;
        int32_t var2;
        int32_t temperature;
    ...
    float BME280SPI::compensatePressure(const unsigned long adc_P)
    {
        int64_t var1;
        int64_t var2;
        int64_t var3;
    ...
    int BME280SPI::compensateHumidity(const unsigned long adc_H)
    {
        int32_t var1;
        int32_t var2;
        int32_t var3;
    ...
    esp_err_t BME280SPI::Init(const uint8_t humidityOversampling,
    ...
    int BME280SPI::GetDeviceID()
    {
        return readByteData(ID);
    }

    esp_err_t BME280SPI::SetConfig(const uint8_t config)
    {
        return writeByteData(CONFIG, config);
    }

    esp_err_t BME280SPI::SetConfigStandbyT(const uint8_t standby) // config bits 7, 6, 5  page 30
    {
    ...
    esp_err_t BME280SPI::SetConfigFilter(const uint8_t filter) // config bits 4, 3, 2
    {
    ...
    esp_err_t BME280SPI::SetCtrlMeas(const uint8_t ctrlMeas)
    {
    ...
    esp_err_t BME280SPI::SetTemperatureOversampling(const uint8_t tempOversampling) // ctrl_meas bits 7, 6, 5   page 29
    {
    ...
    esp_err_t BME280SPI::SetPressureOversampling(const uint8_t pressureOversampling) // ctrl_meas bits 4, 3, 2
    {
    ...
    esp_err_t BME280SPI::SetOversampling(const uint8_t tempOversampling, const uint8_t pressureOversampling)
    {
    ...
    esp_err_t BME280SPI::SetMode(const uint8_t mode) // ctrl_meas bits 1, 0
    {
    ...
    esp_err_t BME280SPI::SetCtrlHum(const int humididtyOversampling) // ctrl_hum bits 2, 1, 0    page 28
    {
    ...
    esp_err_t BME280SPI::GetAllResults(BME280ResultData *results)
    {
        esp_err_t status = ESP_OK;
    ...
    esp_err_t BME280SPI::GetAllResults(float *temperature, int *humidity, float *pressure)
    {
        esp_err_t status = ESP_OK;
        SensorRawData resultRaw{};
    ...
    float BME280SPI::GetTemperature(void) // Preferable to use GetAllResults()
    {
    ...
    float BME280SPI::GetPressure(void)
    {
        BME280ResultData results{};
    ...
    int BME280SPI::GetHumidity(void)
    {
        BME280ResultData results{};
    ...
    bool BME280SPI::StatusMeasuringBusy(void) // check status (0xF3) bit 3
    {
        return ((readByteData(STATUS) & 8) == 8) ? true : false;
    }

    bool BME280SPI::ImUpdateBusy(void) // check status (0xF3) bit 0
    {
        return ((readByteData(STATUS) & 1) == 1) ? true : false;
    }

    esp_err_t BME280SPI::Reset(void) // write 0xB6 into reset (0xE0)
    {
        return writeByteData(RESET, 0xB6);
    }

} // namespace CPPBME280SPI
